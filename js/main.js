...
function loadFlights() {
  ...
  //   },i*100);
  // });
  console.log(trips[0].flights)
  $.each(trips[0].flights,function(i,flight){
    drawFlight(flight);
  })
}
function drawFlight(flight) {
  var generator = new arc.GreatCircle(
    obj(airports[flight.origin.airport].coordinates),
    obj(airports[flight.destination.airport].coordinates));
...
function getTweets() {
  ...
  $.getJSON('http://kziel.herokuapp.com/timeline/kziel?count=200&since_id=557472755302821887&exclude_replies=false',function(data){
    twitterData(data);
  })
}
...
function centerPolygon(polygon) {
  ...
  newCoords[0] = (newCoords[0]/4)+(Math.random()*0.0005);
  newCoords[1] = (newCoords[1]/4)+(Math.random()*0.0005);
  return newCoords;
}
...
function obj(coords) { return { y: coords.latitude, x: coords.longitude }; }
